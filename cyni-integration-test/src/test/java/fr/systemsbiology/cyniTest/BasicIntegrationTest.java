 ...
 */
package fr.systemsbiology.cyniTest;
...
public abstract class BasicIntegrationTest {
	...
	@Inject
	protected CyLayoutAlgorithmManager layoutManager;
	@Inject
	protected CyNetworkViewFactory netViewFactory;
	@Inject
	protected TunableSetter tunableSetter;
	
	
	@Inject 
	protected CyCyniAlgorithmManager cyniManager;
	@Inject 
	protected CyCyniMetricsManager metricsManager;
	...
	public Option[] config() {
		...
		String cyniBundleVersion = System.getProperty("cyni.impl.version");
		String cyniApiVersion = System.getProperty("cyni.api.version");
		...
		return options(
				...
				wrappedBundle(mavenBundle().groupId("com.apple.developer").artifactId("AppleJavaExtensions").version("1.6").startLevel(3)),
				...
				mavenBundle().groupId("org.cytoscape").artifactId("swing-util-impl").version(implBundleVersion).startLevel(11),
				mavenBundle().groupId("org.cytoscape").artifactId("work-swing-impl").version(implBundleVersion).startLevel(12),
				...
				mavenBundle().groupId("org.cytoscape").artifactId("command-executor-impl").version(implBundleVersion).startLevel(25),
				mavenBundle().groupId("org.cytoscape").artifactId("swing-application-impl").version(implBundleVersion).startLevel(26),
				//mavenBundle().groupId("org.cytoscape").artifactId("cyni-api").version(cyniApiVersion).startLevel(26),
				mavenBundle().groupId("fr.systemsbiology").artifactId("cyni-app").version(cyniBundleVersion).startLevel(27)
	...
	void checkBasicConfiguration() throws Exception {
		...
		assertNotNull(cyniManager);
		assertNotNull(rootNetManager);
		assertNotNull(layoutManager);
		assertNotNull(netViewFactory);
		assertNotNull(tunableSetter);
